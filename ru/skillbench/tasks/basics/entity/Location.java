 ...
 * В описание входит название места (например, "Россия", "пр. Строителей", "д. 10"),
 * тип (например, страна, улица, дом)
 *  и ссылка на "родительское" место, т.е. на место, частью которого является данное место.<br/>
 * Кроме простых getter/setter-методов, нужно реализовать {@link #getTopLocation()}
 * для поиска вершины иерархии мест,
 ...
 * <li>Класс решения задачи должен быть объявлен так:
 * <code>public class LocationImpl implements Location {   }</code>.</li>
 * <li>Конструкторы объявлять не требуется, но если они есть в классе,
 * то среди них должен быть конструктор без параметров:
 *     <code>public EmployeeImpl() {  }</code></li>
 * <li>Данные должны быть объявлены как private-поля класса.
 * Методы get* / set* должны оперировать с этими полями,
 *     но не должны рассчитывать на то,
 *     что "родительское" место является экземпляром класса <code>LocationImpl</code>.</li>
 * <li>Поскольку родительское место, установленное методом {@link #setParent(Location)},
 * может быть экземпляром другого класса,
 *   в методах {@link #getTopLocation()}, {@link #isCorrect()}, {@link #getAddress()}
 *   нельзя обращаться к полям класса <code>LocationImpl</code>.
 *   Более того, поскольку в интерфейсе <code>Location</code>
...
public interface Location {
     ...
     * @return Место, отличное от <code>null</code>. Если выше в иерархии больше нет мест,
     * возвращает данное место.
     */
    Location getTopLocation();

    /**
     * Проверяет иерархию родительских мест на соответствие их типов: например,
     * город не должен быть частью дома или города.<br/>
     * Пропуски в иерархии допустимы: например, улица может находиться в городе (не в районе),
     * а город - в стране (не в области).
     * @return true если каждое следующее (более высокое) место в иерархии имеет {@link #getType()} меньше,
     * чем у предыдущего места.<br/>
     ...
     *    если оно заканчиваются на точку ('.') или содержит точку среди символов от начала названия
     *    до первого пробела (' ');</li>
     ...
     * </ol>
     * Пример названий с префиксом/суффиксом, удовлетворяющих условию пункта 1:
     * "Московская обл.", "туп. Гранитный", "оф. 321".<br/>
     * Пример названий без префикса/суффикса, НЕ удовлетворяющих этому условию: "Москва", "25 к. 2"<br/>
     * @return адрес, составленный из имен (и типов) всех мест,
     * начиная с данного места до {@link #getTopLocation()}
     */
    String getAddress();

    /**
     * Примечание: существует одноименный метод в классе {@link Object},
     * поэтому, в отличие от остальных методов {@link Location},
     *  IDE автоматически НЕ создаст шаблон реализации этого метода в классе LocationImpl -
     *  следует создать его вручную.
