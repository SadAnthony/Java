 ...
 * <p/>
 * Пример похожей структуры данных:
 ...
 *   т.е. при добавлении дочернего узла (шаблон "lazy initialization").
 *   Это значит, что связанные с children getter-методы
 *   (включая {@link #isLeaf()}) не должны вызывать метод, создающий при необходимости пустую коллекцию.<br/>
 * <p/>
 * ПРИМЕЧАНИЕ<br/>
 * Нежелательно переопределять метод {@link #equals(Object)} в вашем классе:
 * это может привести к ошибкам проверки.<br/>
 *
...
public interface TreeNode {
     ...
     * а) поиск/удаление успешно, если в ней нашелся объект,
     * равный указанному в смысле метода {@link Object#equals(Object)};<br/>
     ...
     * @return <code>true</code> - если узел развернут,
     * <code>false</code></code></code> - если узел "свернут" (collapsed)
    ...
    String getTreePath();
    /**
     * Среди цепочки родительских узлов данного <code>TreeNode</code>
     * метод находит (первый) узел с заданным объектом <code>data</code>.<br/>
     * По соглашению, "цепочка родительских узлов" содержит сам данный узел
     * (то есть, возможно следующее: obj.findParent(*) == obj).<br/>
     * Объекты <code>data</code> должны сравниваться с помощью {@link Object#equals(Object)},
     * а если <code>data == null</code>,
     ...
     * @return Найденный узел. Или <code>null</code> если не было найдено узла,
     * содержащего такой <code>data</code>.
     */
    TreeNode findParent(Object data);
    /**
     * Среди дочерних узлов данного <code>TreeNode</code> метод находит (первый) узел
     * с заданным объектом <code>data</code>.<br/>
